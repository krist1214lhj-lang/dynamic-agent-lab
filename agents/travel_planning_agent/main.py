...
def _planning_summary(destination, days, strategy):
    if days == 1:
        return (
            f"{destination} 당일치기는 이동 부담을 줄이고 핵심 관광지, 식사, "
            "교통 중심으로 압축 구성하는 것이 적합합니다."
        )
    if days == 2:
        return (
            f"{destination} 1박 2일은 첫날 오후와 둘째 날 오전을 중심으로 "
            "이동, 식사, 대표 명소를 균형 있게 배치하는 것이 적합합니다."
        )
    return (
        f"{destination} {strategy['label']} 여행은 권역을 나누어 관광지, 맛집, "
        "행사와 휴식 시간을 균형 있게 구성하는 것이 적합합니다."
    )


def _planning_rules(strategy):
    if strategy["days"] == 1:
        return [
...
def run(input_data):
    ...
    return {
        ...
        "planning_summary": _planning_summary(destination, days, strategy),
        "duration_strategy": strategy,
        "recommended_agent_mix": _recommended_agent_mix(requested_features, days),
        "planning_rules": _planning_rules(strategy),
